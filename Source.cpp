	...
	const int cy)
{
	...
	if (cx > cy) {
		glViewport(rc.left + ((cx - cy) / 2),
			cy - rcViewport.bottom,
			viewSize, viewSize);
	}
	else {
		glViewport(rc.left,
			cy + ((cx - cy) / 2) - rcViewport.bottom,
			viewSize, viewSize);
	}
...
{
	if (numVertices > 12) {
		MessageBoxA(hwndMain, "Не может быть больше 12 по X", "ERROR", ERROR);
		return;
	}
	if (num > 12) {
		MessageBoxA(hwndMain, "Не может быть больше 12 по Y", "ERROR", ERROR);
		return;
	}
